...
func _AppendByte(buff *bytes.Buffer, b byte){
...
func _AppendValue(buff *bytes.Buffer, sig string, val interface{}) (sigOffset int, e os.Error){
	if len(sig) == 0 { return 0, os.NewError("Invalid Signature");}
	...
	case 'a': // ary
		...
		_AppendArray(buff, 1, func(b *bytes.Buffer) {
			if vec,ok := val.(*vector.Vector); ok && vec!=nil{
				for v:=range vec.Iter() { _AppendValue(b, sigBlock, v);}
			}
		});
...
func _GetByte(buff []byte, index int) (byte, os.Error){
	if len(buff) <= index { return 0, os.NewError("index error");}
	return buff[index], nil;
}

func _GetInt32(buff []byte, index int) (int32, os.Error){
	if len(buff) <= index + 4-1 { return 0, os.NewError("index error");}
	var l int32;
...
func _GetBoolean(buff []byte, index int) (bool, os.Error){
	if len(buff) <= index+4-1 { return false, os.NewError("index error");}
	var v int32;
...
func _GetString(buff []byte, index int, size int) (string, os.Error){
	if len(buff) <= (index+size-1) { return "", os.NewError("index error");}
	return string(buff[index:index+size]), nil
}

func _GetStructSig(sig string, startIdx int) (string, os.Error){
	if len(sig) <= startIdx || '(' != sig[startIdx] { return "<nil>",os.NewError("index error");}
	sigIdx := startIdx+1;
	for depth:=0;sigIdx < len(sig);sigIdx++{
		switch sig[sigIdx]{
		case ')':
			if depth == 0 { return sig[startIdx+1:sigIdx], nil;}
			depth--;
		case '(':
			depth++;
		}
