...
func(p *Connection) _SendSync(msg *Message, callback func(*Message)) os.Error {
	...
	p.methodCallReplies[seri] = func(rmsg *Message){ callback(rmsg); recvChan <- 0;};
...
func(p *Connection) Interface(obj *Object, name string) *Interface{
	...
	if obj == nil || obj.intro == nil { return nil;}
...
func(p *Interface) CallMethod(name string, args ...) os.Error{
	...
	if nil == method { return os.NewError("Invalid Method");}
	...
	p.bus._SendSync(msg, func(reply *Message){
		fmt.Println("Method Call Comp:", name);
	});
