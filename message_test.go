...
func TestUnmarshal(t *testing.T){
	...
	if nil != e {t.Error("Unmarshal Failed");}
	if METHOD_CALL != msg.Type { t.Error("#1 Failed :", msg.Type);}
	if "/org/freedesktop/DBus" != msg.Path { t.Error("#2 Failed :", msg.Path);}
	if "org.freedesktop.DBus" != msg.Dest { t.Error("#3 Failed :", msg.Dest);}
	if "org.freedesktop.DBus" != msg.Intf { t.Error("#4 Failed :", msg.Intf);}
	if "Hello" != msg.Member { t.Error("#5 Failed :", msg.Member);}
}
...
func TestMarshal(t *testing.T){
	...
	buff,_ := msg._Marshal();
	if teststr != string(buff) { t.Error("#1 Failed\n", buff, "\n", strings.Bytes(teststr))}
}
